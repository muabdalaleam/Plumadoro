...
type pomodoroRecord struct {
	...
	phaseType        phaseType
	n                uint64 // HACK: this type is just for strconv.ParseUint
	running          bool
	time_            time.Time
}
...
var logPath string = fmt.Sprintf("%s/.plumadoro.csv", cacheDir)
...
func (p *PomodoroModel) save() error {
	...
	// XXX: IDK how the hell this works but i don't give a s***
...
// TODO: please do a better error handling
func (p *PomodoroModel) restore() error {
	...
	p.ProgressBar       = progress.New(
		progress.WithSolidFill(p.loadProgressBarColor()),
	)
